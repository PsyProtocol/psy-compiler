...
mod test_helpers;
use test_helpers::contract::gen_test_contract;

#[derive(Parser, Debug)]
struct BenchmarkRegisterV2Args {
...
async fn main() -> anyhow::Result<()> {
    ...
    run(BenchmarkRegisterV2Args::parse()).await
}
...
pub async fn run(args: BenchmarkRegisterV2Args) -> anyhow::Result<()> {
    run_test3().await
}
