#!/usr/bin/env bash
# Publish a psy-compiler GitHub Release with immutability guarantees.
#
...
: "${DIST_DIR:=dist}"
: "${WORKFLOW_OWNER:=psy-compiler-release-workflow}"
...
shopt -s nullglob
assets=(psy-compiler-"${RELEASE_TAG}"-*.tar.gz SHA256SUMS)
if [[ "${#assets[@]}" -ne 5 ]]; then
...
if "$GH" release view "$RELEASE_TAG" --repo "$GITHUB_REPOSITORY" \
  ...
  "$GH" release create "$RELEASE_TAG" --repo "$GITHUB_REPOSITORY" \
    --verify-tag --draft --title "psy-compiler ${RELEASE_TAG}" --generate-notes
