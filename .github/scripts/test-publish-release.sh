...
mk_payload() {
  ...
  for t in "${triples[@]}"; do
    ...
    local f="psy-compiler-${tag}-${t}.tar.gz"
    printf 'payload for %s\n' "$t" > "$dir/$f"
    ( cd "$dir" && sha256sum "$f" ) >> "$sumfile"
    i=$((i+1))
  done
  if [[ $count -ge 5 ]]; then :; else :; fi
}
...
assert_eq "fresh: sentinel stamped" "1" "$(grep -c 'psy-compiler-release-workflow' "$STORE/v1.2.3/meta.json")"
...
if run_publish "$dist" "v2.0.0" >/dev/null 2>&1; then
  echo "  FAIL published release was mutated" >&2; FAIL=$((FAIL+1))
else
  PASS=$((PASS+1)); echo "  ok   published release refused"
fi
...
if run_publish "$dist" "v3.0.0" >/dev/null 2>&1; then
  echo "  FAIL foreign draft was mutated" >&2; FAIL=$((FAIL+1))
else
  PASS=$((PASS+1)); echo "  ok   foreign draft refused"
fi
...
printf '{"isDraft":true,"body":"<!-- psy-compiler-release-workflow -->\n\nprior notes"}\n' > "$STORE/v4.0.0/meta.json"
dist="${WORK}/d-owned"; mk_full_payload "$dist" "v4.0.0"
if out="$(run_publish "$dist" "v4.0.0" 2>&1)"; then
  PASS=$((PASS+1)); echo "  ok   owned draft resumed"
else
  echo "  FAIL owned draft resume failed: $out" >&2; FAIL=$((FAIL+1))
fi
...
if run_publish "$dist" "v5.0.0" >/dev/null 2>&1; then
  echo "  FAIL short payload accepted" >&2; FAIL=$((FAIL+1))
else
  PASS=$((PASS+1)); echo "  ok   short payload refused"
fi
...
bad="psy-compiler-v6.0.0-aarch64-apple-darwin.tar.gz"
